...
@Component({
  selector: 'app-dealerbrandterms',
  templateUrl: './dealerbrandterms.component.html',
  styleUrls: ['./dealerbrandterms.component.scss']
})
export class DealerbrandtermsComponent implements OnInit {
  ...
  editYESORNOValues(id,termid,termName,value){
    this.yesORnoValues.push(
      this.newEditYNQuantity(id,termid,termName,value));
     
  }
  newEditYNQuantity(id,termid,termName,value):FormGroup{
    return this.formBuilder.group({
      identity:id,
      TermName:termName,
      idx:id,
  ...
  editTextValues(id,termid,termName,value){
    this.textArray.push(
    this.newEditTextQuantity(id,termid,termName,value));
  }
  newEditTextQuantity(id,termid,termName,value):FormGroup{
    return this.formBuilder.group({
      identity:id,
      TermName:termName,
      i:[value],
 ...
 dealeruserid=localStorage.getItem('dealeruserid');
        ...
        if(this.savedBrandTerms.length !=0){
          for(var i=0;i<this.savedBrandTerms.length;i++){
            if(this.savedBrandTerms[i].TermType==1){
              this.editYESORNOValues(this.savedBrandTerms[i].DT_ID,this.savedBrandTerms[i].DT_BT_ID,this.savedBrandTerms[i].TermName,this.savedBrandTerms[i].value);
            }
            else if(this.savedBrandTerms[i].TermType==2){
              this.editTextValues(this.savedBrandTerms[i].DT_ID,this.savedBrandTerms[i].DT_BT_ID,this.savedBrandTerms[i].TermName,this.savedBrandTerms[i].value);
            }
            else if(this.savedBrandTerms[i].TermType==3){
              this.editselectValues(this.savedBrandTerms[i].DT_ID,this.savedBrandTerms[i].DT_BT_ID,this.savedBrandTerms[i].TermName,this.savedBrandTerms[i].DT_VALUE);
  ...
  editselectValues(id,termid,termname,value){
    this.optionArray.push(
      this.newEditSelectQuantity(id,termid,termname,value)
    );
  }
  newEditSelectQuantity(id,termid,termname,value):FormGroup{
    return this.formBuilder.group({
      identity:id,
      TermName:termname,
      opt:value,
    ...
    if (this.termGroupForm.value.textArray.length > 0) {
      ...
      for (var j = 0; j < this.termGroupForm.value.textArray.length; j++) {
        if(this.termGroupForm.value.textArray[j].identity == '')
           this.TermFinalArray.push({ 'brandtermid': this.termGroupForm.value.textArray[j].termid, 'dealerid': this.DealerId, 'dtvalue': this.termGroupForm.value.textArray[j].i, 'status': 'Y', 'updateduserId': this.DealerId });
        else
